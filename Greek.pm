...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Egreek::uc($char[$i]) ne Egreek::lc($char[$i]))) {
            $char[$i] = '[' . Egreek::uc($char[$i]) . Egreek::lc($char[$i]) . ']';
        }
...
http://search.cpan.org/dist/Char-Greek/Greek/JA.pm
