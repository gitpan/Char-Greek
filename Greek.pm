...
######################################################################
#
# Greek - Source code filter to Greek script
#
...
sub Greek::escape_script {
    ...
    if (/^ use Egreek(?:\s+[0-9\.]*)?\s*; $/oxms) {
        ...
        $e_script .= sprintf("use Egreek %s;\n%s", $Egreek::VERSION, $use_re_eval); # require run-time routines version

        # use Greek version qw(ord reverse);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
...
Greek - Source code filter to escape Greek

=head1 SYNOPSIS

  use Greek;
  use Greek version;         --- require version
...
Definitions in Egreek.pm.
  ...
  @{Egreek::not_space}      qr{(?:[^\x09\x0A\x0B\x0C\x0D\x20])}
...
Contrasting is Encode module and describing "use Greek;" on this software,
