...
######################################################################
#
# Char::Greek - Source code filter to escape Greek script
#
# http://search.cpan.org/dist/Char-Char::Greek/
#
...
BEGIN { CORE::require Char::Egreek; }
...
my $function_ord;         # ord()   to ord()   or Char::Greek::ord()
my $function_ord_;        # ord     to ord     or Char::Greek::ord_
my $function_reverse;     # reverse to reverse or Char::Greek::reverse
my $function_getc;        # getc    to getc    or Char::Greek::getc
...
END
    ...
    print Char::Greek::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        ...
        if (eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
            ...
            Char::Egreek::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e";
        }
        ...
        my $e_script = Char::Greek::escape_script($filename);
        print {$fh} $e_script;
    ...
    my $fh = gensym();
    Char::Egreek::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e";
...
sub Char::Greek::escape_script {
    ...
    # read Greek script
    my $fh = gensym();
    Char::Egreek::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script";

    if (/^ use Char::Egreek(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Char::Egreek %s;\n", $Char::Greek::VERSION); # require run-time routines version

        # use Char::Greek version qw(ord reverse getc);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
        ...
        if (s/^ \s* use \s+ Char::Greek \s* ([^;]*) ; \s* \n? $//oxms) {
            ...
            if ($list =~ s/\A ([0-9]+\.[0-9]+) \.0 \s* //oxms) {
                my $version = $1;
                if ($version ne $Char::Greek::VERSION) {
                    my @file = grep -e, map {qq{$_/Char/Greek.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Char/Greek.pm $version, but @{[__FILE__]} is version $Char::Greek::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Char/Egreek.pm $version, but \\\$Char::Egreek::VERSION is \$Char::Egreek::VERSION" if \$Char::Egreek::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ([0-9]+(?:\.[0-9]*)) \s* //oxms) {
                my $version = $1;
                if ($version > $Char::Greek::VERSION) {
                    die "Script $0 required Char/Greek.pm $version, but @{[__FILE__]} is only version $Char::Greek::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Char::Greek::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Char::Greek::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Char::Greek::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Char::Greek::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
# functions of package Char::Egreek
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Char::Greek::re_t=%s; while($Char::Greek::re_t =~ %s){%s local $^W=0; local $Char::Greek::re_r=%s; %s$Char::Greek::re_t="$`$Char::Greek::re_r$'"; pos($Char::Greek::re_t)=length "$`$Char::Greek::re_r"; } return $Char::Greek::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::Greek::re_r=eval $Char::Greek::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Char::Greek::re_r=%s; %s"%s$Char::Greek::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Char::Greek::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::Greek::re_r=%s; %s%s="$`$Char::Greek::re_r$'"; pos(%s)=length "$`$Char::Greek::re_r"; $Char::Greek::re_n++} return %s$Char::Greek::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Char::Greek::re_r=%s; %s%s="%s$Char::Greek::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Char::Greek::re_r=%s; %s%s="%s$Char::Greek::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::Greek::re_r=eval $Char::Greek::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
=item * You'll have to download Char/Greek.pm and Char/Egreek.pm and put it in your perl lib directory.

=item * You'll need to write "use Char::Greek;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Char::Greek;
  use Char::Greek ver.sion;             --- require minimum version
  use Char::Greek ver.sion.0;           --- expects version (match or die)

  # "no Char::Greek;" not supported

  or

  $ perl Char/Greek.pm Greek_script.pl > Escaped_script.pl.e
...
Char::Greek software is "middleware" between perl interpreter and your Perl script
written in Greek.
...
By "use Char::Greek;", it automatically interpret your script as Greek. The various
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Char/Greek.pm, Char/Egreek.pm
  +---------------------------------------+
...
expand wildcard arguments supplied to programs. Instead, import of Char/Egreek.pm
works well.
...
=head1 Software Composition
   ...
   Char/Greek.pm               --- source code filter to escape Greek
   Char/Egreek.pm              --- run-time routines for Char/Greek.pm
...
You need write 'use Char::Greek;' in your script.

  ---------------------------------
  Before      You do
  ---------------------------------
  (nothing)   use Char::Greek;
  ---------------------------------

=head1 Calling 'Char::Egreek::ignorecase()' (Char/Greek.pm provides)

Char/Greek.pm applies calling 'Char::Egreek::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Char::Egreek::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Char/Egreek.pm provides)
...
=head1 Un-Escaping \ of \N, \p, \P and \X (Char/Greek.pm provides)

Char/Greek.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char/Greek.pm and Char/Egreek.pm provide)

Insert 'Char::Egreek::' at head of function name. Char/Egreek.pm provides your script Char::Egreek::*
functions.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Char::Egreek::lc        Character
  lcfirst     Char::Egreek::lcfirst   Character
  uc          Char::Egreek::uc        Character
  ucfirst     Char::Egreek::ucfirst   Character
  fc          Char::Egreek::fc        Character
  chr         Char::Egreek::chr       Character
  glob        Char::Egreek::glob      Character
  -------------------------------------------
...
=head1 Un-Escaping bytes::* Functions (Char/Greek.pm provides)

Char/Greek.pm removes 'bytes::' at head of function name.
...
=head1 Escaping Built-in Standard Module (Char/Egreek.pm provides)

Char/Egreek.pm does "BEGIN { unshift @INC, '/Perl/site/lib/Char::Greek' }" at head.
Store the standard module modified for Char::Greek software in this directory to
override built-in standard modules.

=head1 Escaping Standard Module Content (You do)

You need copy built-in standard module to /Perl/site/lib/Char::Greek and change
'use utf8;' to 'use Char::Greek;' in its. You need help yourself for now.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Char/Egreek.pm provides these
  functions.
...
Char::Greek software remains one language and one interpreter.
