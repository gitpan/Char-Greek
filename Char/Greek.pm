...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Char::Egreek::uc($char[$i]) ne Char::Egreek::lc($char[$i]))) {
            $char[$i] = '[' . Char::Egreek::uc($char[$i]) . Char::Egreek::lc($char[$i]) . ']';
        }
...
http://search.cpan.org/dist/Char-Char::Greek/Char::Greek/JA.pm
