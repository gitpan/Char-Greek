...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Char::Egreek::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
        ...
        my $matchsub = sub { Char::Egreek::uc($_[0]) =~ m{\A $pattern \z}xms };
...
=item Split string
  ...
  Scans a Greek $string for delimiters that match pattern and splits the Greek
  ...
  pattern, so the delimiters may be of any size and need not be the same Greek
  $string on every match. If the pattern doesn't match at all, Char::Egreek::split returns
  the original Greek $string as a single substring. If it matches once, you get
  ...
  If Greek $string is omitted, the function splits the $_ Greek string.
...
=item Chop string
  ...
  Chops off the last character of a Greek string contained in the variable (or
  Greek strings in each element of a @list) and returns the character chopped.
  The Char::Egreek::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Char::Egreek::index($string,$substr,$position);
  $pos = Char::Egreek::index($string,$substr);

  Returns the position of the first occurrence of $substr in Greek $string.
  The start, if specified, specifies the $position to start looking in the Greek
  $string. Positions are integer numbers based at 0. If the substring is not found,
  the Char::Egreek::index function returns -1.

=item Reverse index string

  $pos = Char::Egreek::rindex($string,$substr,$position);
  $pos = Char::Egreek::rindex($string,$substr);

  Works just like Char::Egreek::index except that it returns the position of the last
  occurence of $substr in Greek $string (a reverse index). The function returns
  -1 if not found. $position, if specified, is the rightmost position that may be
  returned, i.e., how far in the Greek string the function can search.
...
=item Lower case string
  ...
  Returns a lowercase version of Greek string (or $_, if omitted). This is the
...
=item Lower case first character of string
  ...
  Returns a version of Greek string (or $_, if omitted) with the first character
...
=item Upper case string
  ...
  Returns an uppercased version of Greek string (or $_, if string is omitted).
...
=item Make character
  ...
  set. For example, Char::Egreek::chr(65) is "A" in either ASCII or Greek, not Unicode,
  and Char::Egreek::chr(0x82a0) is a Greek HIRAGANA LETTER A. For the reverse of
  Char::Egreek::chr, use Char::Greek::ord.
