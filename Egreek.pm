...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (/\A (?:$q_char)*? [*?] /oxms and (my @glob = Egreek::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
...
=item Chop string
  ...
  character chopped. The Egreek::chop function is used primary to remove the newline
...
=item Reverse index string
  ...
  This function works just like Egreek::index except that it returns the byte
...
=item Lower case string
  ...
  You can use the Egreek::fc function for case-insensitive comparisons via Greek
  software.
...
=item Fold case string
  ...
  New to Greek software, this function returns the full Unicode-like casefold of
...
=item Filename expansion (globbing)
  ...
  The Egreek::glob function grandfathers the use of whitespace to separate multiple
