...
BEGIN {
    ...
    if ($] < 5.006) {
        ...
        if ($^O ne 'MacOS') {
            *CORE::GLOBAL::binmode =
            *CORE::GLOBAL::binmode = \&Egreek::binmode;
        }
        *CORE::GLOBAL::open    =
        *CORE::GLOBAL::open    = \&Egreek::open;
    }
}
...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (m/\A (?:$q_char)*? [*?] /oxms and (my @glob = Egreek::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
...
sub _octets {
    ...
    if ($length == 1) {
        ...
        if (((caller(1))[3] ne 'Egreek::_octets') and ($modifier =~ m/i/oxms)) {
            if ($a == $z) {
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Egreek::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
        ...
        my $matchsub = sub { Egreek::uc($_[0]) =~ m{\A $pattern \z}xms };
...
=item Split string
  ...
  $string on every match. If the pattern doesn't match at all, Egreek::split returns
...
=item Chop string
  ...
  The Egreek::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Egreek::index($string,$substr,$position);
  $pos = Egreek::index($string,$substr);
  ...
  the Egreek::index function returns -1.

=item Reverse index string

  $pos = Egreek::rindex($string,$substr,$position);
  $pos = Egreek::rindex($string,$substr);

  Works just like Egreek::index except that it returns the position of the last
...
=item Make character
  ...
  set. For example, Egreek::chr(65) is "A" in either ASCII or Greek, and
  Egreek::chr(0x82a0) is a Greek HIRAGANA LETTER A. For the reverse of Egreek::chr,
  use Greek::ord.
...
=item binary mode (Perl5.6 emulation on perl5.005)
  ...
  Egreek::binmode(FILEHANDLE, $disciplines);
  Egreek::binmode(FILEHANDLE);
  Egreek::binmode($filehandle, $disciplines);
  Egreek::binmode($filehandle);
...
=item open file (Perl5.6 emulation on perl5.005)
  ...
  $rc = Egreek::open(FILEHANDLE, $mode, $expr);
  $rc = Egreek::open(FILEHANDLE, $expr);
  $rc = Egreek::open(FILEHANDLE);
  $rc = Egreek::open(my $filehandle, $mode, $expr);
  $rc = Egreek::open(my $filehandle, $expr);
  $rc = Egreek::open(my $filehandle);
  ...
  * three arguments
  ...
  {
      my $fh;                          # (uninitialized)
      Egreek::open($fh, ">logfile")     # $fh is autovivified
  ...
  Egreek::open my $fh, ">logfile" or die ...
  ...
  Egreek::open(LOG, ">", "logfile") or die "Can't create logfile: $!";
  ...
  $path =~ s#^([ ])#./$1#;
  Egreek::open (FH, "< $path\0") or die "can't open $path: $!";
  ...
  Egreek::open(FH, "<", $path) or die "can't open $path: $!";
  ...
  Egreek::open(FH, "<:raw", $path) or die "can't open $path: $!";
  ...
  Egreek::open(FH, "<:crlf:encoding(Greek)", $path) or die "can't open $path: $!";
